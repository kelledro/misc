...
cfnConnection = boto.cloudformation.connect_to_region(region)
...
stackResult = cfnConnection.describe_stacks(stackName)
print("."),
...
print("done.")
for output in stackResult[0].outputs:
    if output.key == "esAddress":
        address = output.value
print "Wait a few minutes for elasticsearch to start and then visit %s" % address
